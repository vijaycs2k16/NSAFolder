...
import nexrise.publication.in.nexrise.Gallery.FullImageActivity;
import nexrise.publication.in.nexrise.Gallery.SelectedImagesActivity;
...
public class ImageSelectActivity extends HelperActivity {
    ...
    int SELECTED_IMAGE_ACTIVITY = 100;
    int EDIT_IMAGE_RESULT = 101;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
       ...
       /* Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        ...
        forCallback = intent.getStringExtra("For callback");
        ...
        if(from.equals("AlbumSelectFragment")) {
        ...
        } else if(from.equals("AlbumSelectActivity") || from.equals("AttachmentActivity")) {
    ...
    private void sendIntent() {
        if (forCallback == null) {
            if (from.equals("AlbumSelectActivity")) {
                Log.v("selected ","images "+getSelected());
                Intent intent = new Intent(ImageSelectActivity.this, SelectedImagesActivity.class);
                intent.putParcelableArrayListExtra(Constants.INTENT_EXTRA_IMAGES, getSelected());
                setResult(RESULT_OK, intent);
                startActivityForResult(intent, EDIT_IMAGE_RESULT);
            } else if (from.equals("AlbumSelectFragment")) {
                Intent intent = new Intent();
                intent.putParcelableArrayListExtra(Constants.INTENT_EXTRA_IMAGES, getSelected());
                setResult(RESULT_OK, intent);
                finish();
            } else if (from.equals("AttachmentActivity")) {
                Intent intent = new Intent();
                intent.putParcelableArrayListExtra(Constants.INTENT_EXTRA_IMAGES, getSelected());
                setResult(RESULT_OK, intent);
                finish();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if(requestCode==SELECTED_IMAGE_ACTIVITY && resultCode==RESULT_OK){
            Intent intent = new Intent();
            intent.putParcelableArrayListExtra(Constants.INTENT_EXTRA_IMAGES,data.getParcelableArrayListExtra("photoset"));
            setResult(RESULT_OK,intent);
            finish();
