...
function dynamicTable(result) {
    ...
    var headers = '<thead class="samplePrint"><tr><th colspan="1">Roll No</th><th colspan="1">Name</th>';
    var tbody = '<tbody class="samplePrint">'
    ...
    for (var i = 0; i < students.length; ++i) {
        var rollNo = ((students[i].rollNo != null)) ? students[i].rollNo : '-';
        tbody += '<tr><td>' + rollNo + '</td><td>' + students[i].firstName + '</td>'
        for (var j = 0; j < students[i].student.length; ++j) {
            if (students[i].student[j].grade == 'F') {
                tbody += '<td>' + students[i].student[j].max_marks_grade + '</td><td class="sample" style="background-color: red">' + students[i].student[j].grade + '</td>'
            ...
            if(j == students[i].student.length - 1 ) {
                if(students[i].oGrade == 'F') {
                    tbody += '<td>' + students[i].tMarks + '</td><td>' + (students[i].oMarks).toFixed(2) + '</td><td>' + students[i].percentage + '</td><td class="sample" style="background-color: red">' + students[i].oGrade + '</td><td>' + students[i].application + '</td><td>' + students[i].Attendance + '</td></tr>'
                } else if(students[i].student[j].max_marks_grade == -1){
    ...
    //$("#table").css("border", "2px solid #000")
