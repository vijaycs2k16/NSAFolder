...
export class DualListComponent implements DoCheck, OnChanges {
	...
	moveAll(source:BasicList) {
		this.selectAll(source);
		this.moveItem(source, this.confirmed);
	}

	moveLeftAll(confirmed:BasicList) {
		this.selectAll(confirmed);
		this.moveItem(confirmed, this.available);
	}
