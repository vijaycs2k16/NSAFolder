define([
    'Backbone',
    'Underscore',
    'text!templates/VAttendance/monthTemplate.html',
    'views/VAttendance/StatisticsView',
    'moment'
    ...
    var MonthView = Backbone.View.extend({
        ...
        generateMonthData: function (currentInterval, present) {
            var self = this;
            var monthYear;
            var monthNumber;
            var startOfMonth;
                        ...
                        for (k = j + startOfMonth - 1; k <= j + startOfMonth - 1; k++) {
                            ...
                            switch (vacArray[j]) {
                                case 'V':
                                    ...
                                    break;
                                case 'H':
                                    self.halfDays = self.halfDays + 0.5;
                                    break;
                                case 'M':
                                    self.medicalDays++;
                                    break;
            ...
            self.daysLeave = self.vacationDays + self.halfDays +  self.medicalDays + self.sickDays + self.educationDays;
            self.workingDays = endYear.diff(startYear, 'days') - self.weekend;

            if(present) {
                if(currentInterval !== 'Line Year') {
                    self.daysLeave = present.leaveDays;
                    self.vacationDays = present.vacation;
                    self.halfDays = present.half;
                    self.medicalDays = present.medical;
                    self.sickDays = present.sick;
                    self.educationDays = present.education;
                    self.workingDays = present.workingDays;
                }
            }
        },
        ...
        render: function (options) {
            ...
            self.generateMonthData(self.year, present);
