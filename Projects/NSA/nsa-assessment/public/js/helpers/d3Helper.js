define(['d3',
    ...
    function drawVerticalChart(options) {
        ...
        svg.selectAll('.hover')
            ...
            .on('mouseover', function (d) {
                ...
                var name = d[2] ? d[2] : d[0]
                tooltip.html('<div>' + name + '</div>' +
                    '<div>' + ' ' + symbol + helpers.currencySplitter(d[1].toFixed(afterDot)) + '</div>')
        ...
        svg.selectAll("text.hover")
            ...
            .text(function (d) {
                if(d[0]) {
                    return symbol + helpers.currencySplitter(d[1].toFixed(afterDot))
                }
    ...
    function drawPieChart(options) {
        ...
        arcs = svg.selectAll("g.arc")
            ...
            .attr("transform", "translate(" + (width / 2 - 50 + margin.left) + "," + (height / 2 + margin.top) + ")");
    ...
    function drawDonutChart(options) {
        ...
        arcs = svg.selectAll("g.arc")
            ...
            .append("g")
            .attr("class", "arc")
            .attr("transform", "translate(" + (width / 2  - 30 + margin.left) + "," + (height / 2 + margin.top) + ")");
        ...
        var legendG = svg.selectAll(".legend") // note appending it to mySvg and not svg to make positioning easier
            ...
            .enter().append("g")
            .attr("transform", function (d, i) {
                return "translate(" + (width - 100) + "," + (i * 15 + 20) + ")"; // place each legend on the right and bump each one down 15 pixels
            })
            .attr("class", "legend");
        ...
        svg.append('text')
            ...
            .attr({
                ...
                x: function () {
                    return width / 2 - 30 + margin.left - d3.select(this).node().getBBox().width / 2;
                },
