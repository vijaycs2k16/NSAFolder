...
//    var SettingsService = require('../services/settings')(models, event);
    ...
    function getNameAndDepartment(db, query, callback) {
        ...
        if (query) {
            ...
            if (query.centerId) {
                centerMatchQuery.center =objectId(query.centerId);
            }
        }

        Model.aggregate([
            {
                $match: centerMatchQuery
            },{
                $lookup: {
                    from        : 'Department',
        ...
        getNationality = function (pCb) {
            ...
            employee
                .aggregate([{
                    ...
                    nationality = nationality.length > 0 ? nationality[0].nationality : {}
                    pCb(null, nationality);
                });
        };

        parallelTasks = [getEmployee, getTransfer, getNationality];
                ...
                switch (viewType) {
                    case ('list'):
                        ...
                        projectAfterRoot = {
                            ...
                            dateOfJoin      : '$root.dateOfExists',
                            dateOfExists    : '$root.dateOfJoin',
                            skype           : '$root.skype',
                    ...
                    case ('thumbnails'):
                        ...
                        projectAfterRoot = {
                            ...
                            department     : '$root.department',
                            dateBirth      : '$root.dateBirth',
                            dateOfJoin     : '$root.dateOfJoin',
                            dateOfExists   : '$root.dateOfExists',
                            isEmployee     : '$root.isEmployee',
                            total          : 1,
                            'editedBy.date': '$root.editedBy.date'
                        };
    ...
    this.getAssignees = function (req, res, next) {
        var Model = models.get(req.session.lastDb, 'Employees', EmployeeSchema);
        var matchArr = [1524552625000,1524552351000,1522230115000,1522233094000];
        var tasks = [1524553113000,1524552941000,1524552846000];
        var viewType = req.query.viewType;

        switch (viewType) {
            case 'leads':
                matchArr = req.query.profileId ? [+req.query.profileId] : matchArr;
                break;
            case 'opportunities':
                matchArr = req.query.profileId ? [+req.query.profileId] : matchArr;
                break;
        ...
        Model.aggregate([
            ...
            {
                $match: {$or: [{'user.profile': {$in: matchArr}}]}
            }
        ], function(err, result){
            if(err){
                next(err);
    ...
    this.create = function (req, res, next) {
        ...
        body.dbName = dbName;
        body.dataOfJoin = getDate(body.dateOfJoin)
        body.dateOfExists = getDate(body.dateOfExists)
        if (body.dateBirth) {
        ...
        function create(body, callback) {
            ...
            function generateEmpId(waterfallCb) {
                Model.find({ employeeId: { $ne: null }}).sort({ creationDate: -1 }).exec(function (err, result) {
                    if (err) {
                        return waterfallCb(err);
                    }
                    if(!_.isEmpty(result)) {
                        var empId = result[0].employeeId;
                        empId =  empId.split('VPE');
                        body.employeeId = 'VPE' + '0'.repeat(4 - empId.length) + (+empId[1] + 1);
                    }
                    waterfallCb(null, body);
                });
            }

            function employeeCreator(data, waterfallCb) {
                EmployeeService.create(data, waterfallCb);
            }
            ...
            function userCreator(employee, profileId, waterfallCb) {
                ...
                UserService.create(_user, function (err, user) {
                        if (err) {
                            if (err.code === 11000) {
                                err.message = err.message.includes('email') ? 'Email ID already Exists' : 'Phone No already Exists';
                            }
                            return waterfallCb(err);
                        }
            ...
            waterfallTasks = [generateEmpId, employeeCreator];
    ...
    this.createEmployee = function (req, data, next) {
        var dbName = req.session.lastDb;
        var Model = models.get(dbName, 'Employees', EmployeeSchema);
        var userId = req.session.uId;
        var currentDate = new Date();
        var body = data.empObj;
        var err;
        var noteObj;
        body.dbName = dbName;
        if (body.dateBirth) {
    ...
    this.createEmployeeTransfer = function(req, data, next) {
        createEmployeeTransferObj(req, data.empSalaryObj, function (err, data) {
            next(err, data)
        })
    }

    function createEmployeeTransferObj(req, body, next) {
        ...
        transfer.save(function (err, result) {
            ...
            next(null, {success: 'A new Transfer create success'/* , data: result*/})
        });
    }
    exports.createEmployeeTransferObj = createEmployeeTransferObj

    this.createTransfer = function (req, res, next) {
        var body = req.body;
        createEmployeeTransferObj(req, body, function (err, data) {
            if (err) {
                return next(err);
            }
    ...
    this.getEmpTaxanomy = function (req, res, next) {
        /*var userId = req.session ? req.session.uId : null;*/

        getEmpTaxanomyDetails(req, "", function (err, data) {
            if (err) {
                return next(err);
            }
        
            var datobj = {"id": "1", "label": "All", "collapsed": false, "selected": false, "subselector": true, "children": !_.isEmpty(data) ? data[0].data : null}
    ...
    this.getEmpLeaveReport = function (req, res, next) {

        async.parallel({
            months : getEmpAttendanceByMonth.bind(null, req),
            student: getEmpAttendance.bind(null, req)
        }, function (err, result) {
    ...
    function getEmpAttendanceByMonth(req, callback) {
    ...
    function getEmpAttendance(req, callback) {
        var queryParams = req.query;
        if(queryParams.centerId) {
            var Model = models.get(req.session.lastDb, 'Employees', EmployeeSchema);
            Model.aggregate([
                    {
                        "$match": { $or: [
                            {"center": queryParams.centerId},
    ...
    function getEmpTaxanomyDetails(req, userId, callback) {
        var Model = models.get(req.session.lastDb, 'Employees', EmployeeSchema);
        Model.aggregate([
                {
                    "$lookup": {
                ...
                {$group: {_id: {center:"$center"}, "item": { $addToSet:  {"id": "$employees._id", "collapsed": false, "selected": false, "subselector": true, "value": "$employees.workPhones.mobile","label": "$employees.name.first"}}}},
                {$group: {_id: "", data: {$addToSet:  {"id": "$_id.center._id", "label":"$_id.center.centerName", "collapsed": false, "selected": false, "subselector": true, "children":"$item" }}}}

            ], callback
        );
    }

    this.getUpdateEmpTaxanomyDetails = function (req, res, next) {
        var userId = req.session ? req.session.uId : null;

        async.waterfall([
            getEmpTaxanomyDetails.bind(null, req, ""),
            async.apply(getEmpNotificationByNId, req),
            constructJson.bind()
        ], function (err, result) {
            if (err) {
                return next(err);
    ...
    function getEmpNotificationByNId(req, data, callback) {
        var dataObj = {};
        dataObj.employees = data;
        var id = req.params.id;
        var Model = models.get("CRM", 'EmployeeNotifications', EmpNotificationSchema);
        Model.find({notificationId: mongoose.Types.ObjectId(id)}, function (err, notifications) {
            dataObj.notification = JSON.parse(JSON.stringify(notifications));
            callback(err, dataObj)
        })
    }

    function constructJson(data, callback) {
        var taxJson = data.employees[0].data;
        if(taxJson.length > 0 ){
            _.forEach(taxJson, function (child1, index) {
                if(child1.children.length > 0) {
                    _.forEach(child1.children, function (child2) {
                        var datafound = _.filter(data.notification, {"employee": child2.id.toString()});
