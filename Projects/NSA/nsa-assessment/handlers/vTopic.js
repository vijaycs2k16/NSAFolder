...
var Module = function (models) {
    ...
    this.create = function (req, res, next) {
        ...
        /*var topicObj = topic[0].topics;
        topicObj = _.forEach(topicObj, function (o) { o._id = objectId() });
        _.forEach(topic, function (obj) {
           obj.topics = topicObj;
        });*/
        Model.remove({"subject":  objectId(topic[0].subject)}, function (err, data) {
            Model.insertMany(topic, function (err, result) {
                if (err) {
                    return next(err);
                }
    ...
    this.getTopicsByBatch = function (req, res, next) {
        ...
        var Subject = models.get(req.session.lastDb, 'Topic', topicSchema);
        var data = req.query;
        var query;
    ...
    this.getBySubjectCourseId = function (req, res, next) {
        ...
        if(data.classId) {
            findQuery.classDetail =  mongoose.Types.ObjectId(data.classId)
        }

        query = topic.find(findQuery).populate({path : 'subject'}).populate({path : 'course'});
        query.exec(function (err, response) {
            if (err) {
                return next(err);
            }
    ...
    this.getBySubjectClassDetailId = function (req, res, next) {
        var Subject = models.get(req.session.lastDb, 'SubjectTopics', SubjectTopicSchema);
        var data = req.query;
        var query, findQuery;
        query = Subject.find({subject: data.subject}).populate({path : 'subject'}).populate({path : 'classDetail'});
        query.exec(function (err, response) {
            if (err) {
                return next(err);
            }
    ...
    this.remove = function (req, res, next) {
        ...
        var Journal = models.get(req.session.lastDb, 'Topic', topicSchema);
    ...
    this.update = function (req, res, next) {
        var topicListModel = models.get(req.session.lastDb, 'Topic', topicSchema);
        var body = req.body;
