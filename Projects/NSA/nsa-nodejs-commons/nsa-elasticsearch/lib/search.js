...
var attendanceDetailsConverter = require('./converters/attendance.converter');
...
search.searchAttendances = function(req, searchParams, callback) {
    client.getMaster.search(searchParams, function (error, response, status) {
        callback(error,  attendanceDetailsConverter.attendanceObjs(req, response), status);
    });
};
...
search.getStudEmpSuggestions = function(searchParams, callback) {
    client.getMaster.search(searchParams, function (error, response, status) {
        callback(error, userConverter.convertSuggestStudEmp(response), status);
    });
};
