...
Timetable.getTimetableByClssAndSections = function(req, callback) {
    var headers = baseService.getHeaders(req);
    var queryObject = {
        tenant_id : models.timeuuidFromString(headers.tenant_id),
        school_id : models.uuidFromString(headers.school_id),
        class_id : models.uuidFromString(req.body.classId),
        sub_emp_association: {'$contains': req.params.id ? req.params.id : headers.user_id},
        academic_year: headers.academic_year
    }
    models.instance.SchoolTimetable.find(queryObject,{allow_filtering : true}, function(err, result){
        result = JSON.parse(JSON.stringify(result));
        callback(err, timetableConverter.classSectionData(req, result));
    })
};
...
Timetable.getTimetableByEmpClass = function(req, callback) {
    var headers = baseService.getHeaders(req);
    var queryObject = {
        tenant_id : models.timeuuidFromString(headers.tenant_id),
        school_id : models.uuidFromString(headers.school_id),
        sub_emp_association: {'$contains':  headers.user_id},
