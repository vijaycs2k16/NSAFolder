define([
    ...
    var EmployeesCollection = Parent.extend({
        ...
        initialize: function (options) {
            ...
            options = options || {};
            options.contentType = 'Opportunities'
            ...
            if (response.data) {
                if (response.data.next7days) {
                    _.map(response.data.next7days, function (employee) {
                        employee.followDate = common.utcDateToLocaleDate(employee.followDate, true);
                        return employee;
                    });
                }
                ...
                if (response.data.overdue) {
                    _.map(response.data.overdue, function (employee) {
                        employee.followDate = common.utcDateToLocaleDate(employee.followDate);
                        return employee;
                    });
                }
                if (response.data.today) {
                    _.map(response.data.today, function (employee) {
                        employee.followDate = common.utcDateToLocaleHours(employee.followDate);
                        return employee;
                    });
                }
            }
