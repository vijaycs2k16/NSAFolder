...
var Filters = function (models) {
    ...
    this.getEmployeesFilters = function (req, res, next) {
        ...
        if(req.session.profileId == 1524553140000) {
            queryObject.$and = [];
            queryObject.$and.push({'department._id': mongoose.Types.ObjectId("5a803fae43c1fb357f38ae80")});
        }
    ...
    this.getUserFilters = function (req, res, next) {
        var lastDB = req.session.lastDb;
            ...
            },
            {
                ...
                $group: {
                    ...
                    login       : {
                        $addToSet: {
                            _id : '$_id',
                            name: {$ifNull: ['$login', 'None']}
                        }
                    },
                    email       : {
                        $addToSet: {
                            _id : '$_id',
                            name: {$ifNull: ['$email', 'None']}
                        }
                    },
                    department : {
                        $addToSet: {
                            _id : {$ifNull: ['$department._id', '']},
                            name: {$ifNull: ['$department.name', 'None']}
                        }
                    },
                    profile: {
                        $addToSet: {
                            _id : {$ifNull: ['$profile._id', '']},
                            name: {
                                $ifNull: ['$profile.profileName', 'None']
                            }
                        }
                    },
                }
        ...
        aggregation.exec(function (err, result) {
            ...
            result.filterInfo = FILTER_CONSTANTS.Users;
    ...
    this.getStudentFilters = function (req, res, next) {
        var lastDB = req.session.lastDb;
        var studentSchema = mongoose.Schemas.Student;
        var UserModel = models.get(req.session.lastDb, 'Student', studentSchema);
        var pipeLine;
        var aggregation;
        var matchQuery = req.session.profileId === 1522230115000 ?  { center: objectId(req.session.cId) } : {};
            ...
            },
            {
                ...
                $group: {
                    ...
                    center       : {
                        $addToSet: {
                            _id : '$center._id',
                            name: {$ifNull: ['$center.centerName', 'None']}
                        }
                    },
                    studentEmail       : {
                        $addToSet: {
                            _id : '$studentEmail',
                            name: {$ifNull: ['$studentEmail', 'None']}
                        }
                    },
                    studentName       : {
                        $addToSet: {
                            _id : '$_id',
                            name: {$ifNull: ['$studentName', 'None']}
                        }
                    },
                    course : {
                        $addToSet: {
                            _id : '$course._id',
                            name: {$ifNull: ['$course.courseName', 'None']}
                        }
                    },
                }
            }];
    ...
    this.getQuestionFilters = function (req, res, next) {
        var lastDB = req.session.lastDb;
        var questionSchema = mongoose.Schemas.Question;
        var Question = models.get(req.session.lastDb, 'Question', questionSchema);
        var pipeLine;
        var aggregation;
        ...
        pipeLine = [
            ...
            },
            {
...

             {
                $project: {
                    ...
                    point           : 1,
                    topic           : 1,
                    topicObjs       : {$arrayElemAt: ['$topics', 0]},
                    topics          : {$arrayElemAt: ['$topics.topics', 0]},
                    classDetail     : {$arrayElemAt: ['$classDetail', 0]},
                    date            : 1,
            ...
            {
              $unwind : {
                  path : '$topics',  preserveNullAndEmptyArrays: true
              }
            },
            {
                $group: {
                    ...
                    classDetail : {
                        $addToSet:{
                            _id : '$classDetail._id',
                            name : '$classDetail.className'
                        }
                    }
                }
            }];

        aggregation = Question.aggregate(pipeLine);
        ...
        aggregation.exec(function (err, result) {
            ...
            result.filterInfo = FILTER_CONSTANTS.VQuestionBank;
            res.status(200).send(result);
        });
    };
    ...
    function topicObjs(data) {
        ...
        if(!_.isEmpty(data)) {
            try {
                ...
                _.forEach(data.topic, function (value, key) {
                    if(!_.isEmpty(value._id)){
                        var topics = _.filter(value.name, {'_id': value._id})
                        if(topics.length > 0) {
    ...
    this.getEnrollmentFilters = function (req, res, next) {
        var lastDB = req.session.lastDb;
        var Model = models.get(lastDB, 'StudentFeeDetails', studentFeeDetails);
        var pipeLine;
        var aggregation;
                ...
                $group: {
                    ...
                    student : {
                        $addToSet: {
                            _id : '$student._id',
                            name: {
                                $concat: ['$student.studentName', ' ', '$student.lastName']
                            }
                        }
                    },
                    registerNo : {
                        $addToSet: {
                            _id : '$student._id',
                            name: {$ifNull: ['$student.registrationNo', 'None']}
                        }
                    },
                    course : {
                        $addToSet: {
                    ...
                    batch : {
                        $addToSet: {
                            _id : '$batch._id',
                            name: {$ifNull: ['$batch.batchName', 'None']}
                        }
                    }
                }
            }];
        ...
        aggregation.exec(function (err, result) {
            ...
            result.filterInfo = FILTER_CONSTANTS.EnrollMent;

            res.status(200).send(result);
        });
    };
    this.getPracticeExamFilters = function (req, res, next){
        var lastDB = req.session.lastDb;
        var practiceScheduleSchema = mongoose.Schemas.PracticeSchedule;
        var Model = models.get(lastDB, 'PracticeSchedule', practiceScheduleSchema);
        var pipeLine;
        var aggregation;
        pipeLine = [
            {
        ...
        aggregation.exec(function (err, result) {
            ...
            result = result.length ? result[0] : {};
            result.filterInfo = FILTER_CONSTANTS.VPracticeExam;
            res.status(200).send(result);
        });

    };

    this.getBundleMoveFilters = function(req, res, next){
        var lastDB = req.session.lastDb;
        var bundleTransSchema = mongoose.Schemas.bundleTransaction;
        var bundleTransaction = models.get(lastDB, 'bundleTransaction', bundleTransSchema);
        ...
        aggregation = bundleTransaction.aggregate(pipeLine);
        ...
        aggregation.exec(function (err, result) {
            ...
            result.filterInfo = FILTER_CONSTANTS.BundleMove;
    ...
    this.getBundleReturnsFilters = function(req, res, next){
        var lastDB = req.session.lastDb;
        var bundleTransSchema = mongoose.Schemas.bundleTransaction;
        var bundleTransaction = models.get(lastDB, 'bundleTransaction', bundleTransSchema);
        var query = {isReturn: true};
        var pipeLine;
        var aggregation;
        ...
        aggregation.exec(function (err, result) {
            ...
            result.filterInfo = FILTER_CONSTANTS.BundleReturns;
    ...
    this.getExamScheduleFilters = function (req, res, next) {
        var lastDB = req.session.lastDb;
        var examSchema = mongoose.Schemas.ExamSchedule;
        var Model = models.get(lastDB, 'ExamSchedule', examSchema);
        var pipeLine;
        var aggregation;
        ...
        aggregation.exec(function (err, result) {
            ...
            result.filterInfo = FILTER_CONSTANTS.VExamSchedule;

            res.status(200).send(result);
        });
    };

    this.getExamStudentResultFilters = function(req, res, next){
        var lastDB = req.session.lastDb;
        var Model = models.get(lastDB, 'Exam', examSchema);
        var pipeLine;
        var aggregation;
        ...
        aggregation.exec(function (err, result) {
            ...
            result.filterInfo = FILTER_CONSTANTS.VExamStudentsResult;
            res.status(200).send(result);
        });
    }
        ...
        pipeLine = [{
            ...
            },
            {
                ...
                $project: {
                    ...
                    taskType   : {$arrayElemAt: ['$taskType', 0]}
                }
            }, {
                $group: {
                    _id     : null,
    ...
    this.getLeadsFilters = function (req, res, next) {
        ...
        if(!headers.isAcess) {
            ...
            query =  { $or: [{salesPerson: objectId(headers.lid)}, {'createdBy.user': objectId(req.session.uId)}] } ;
        }
    ...
    this.getOpportunitiesFilters = function (req, res, next) {
        ...
        pipeLine = [{
            ...
            $group: {
                ...
                salesPerson: {
                    $addToSet: {
                        _id :{ $ifNull:['$salesPerson._id', 'None']},
                        name: {
                            $ifNull: [{
                                $concat: ['$salesPerson.name.first', ' ', '$salesPerson.name.last']
                            }, 'None']
    ...
    this.getStockCorrectionFilters = function(req, res, next){
        var lastDB = req.session.lastDb;
        var stockCorrectionSchema = mongoose.Schemas.stockCorrection;
        var stockCorrection = models.get(lastDB, 'stockCorrections', stockCorrectionSchema);
        var aggregation;
        var pipeLine;
        var query ={_type : 'stockCorrections'}
        pipeLine = [
            {
                $match : query
            },
        ...
        aggregation = stockCorrection.aggregate(pipeLine);
        ...
        aggregation.exec(function (err, result) {
            ...
            result.filterInfo = FILTER_CONSTANTS.stockCorrections;
            res.status(200).send(result);
        });
    };

    this.getStockReturnsFilters = function (req, res, next) {
        var option;
        var lastDB = req.session.lastDb;
        var stockReturnSchema = mongoose.Schemas.stockReturns;
        var stockReturns = models.get(lastDB, 'stockReturns', stockReturnSchema);
        var aggregation;
        var pipeLine;
        var query ={_type : 'stockReturns'}
        pipeLine = [
            {
              $match : query
            },
        ...
        aggregation = stockReturns.aggregate(pipeLine);
        ...
        aggregation.exec(function (err, result) {
            ...
            result.filterInfo = FILTER_CONSTANTS.stockReturns;
            res.status(200).send(result);
        });
    };
