define([
    ...
    'vconstants',
    'Lodash'
], function (Backbone, _, $, ParentView, CreateTemplate, OpportunityModel, common, populate, dataService, AttachView, CONSTANTS, lodash) {
    var CreateView = ParentView.extend({
        ...
        saveItem: function () {
            ...
            var stData = lodash.filter(this.students, {_id: customerId});
            var center = stData.length > 0 ? stData[0].center : null;
            var course = stData.length > 0 ? stData[0].course : null;
            var userName = stData.length > 0 ? stData[0].userName : null;
            var email = stData.length > 0 ? stData[0].studentEmail : '';
            var phones = {};
            var phone = stData.length > 0 ? stData[0].studentPhone : '';
            phones.phone = phone;
        ...
        render: function () {
            ...
            //populate.get('#customerDd', '/vstudents', {"isRegistration":false, count: 10000000}, 'studentName', this, true, true);
            //populate.get2name('#companyDd', CONSTANTS.URLS.CUSTOMERS, {type: 'Company'}, this);
            dataService.getData('/vstudents', {"isRegistration":false, count: 10000000}, function (students) {
                ...
                self.responseObj['#customerDd'] = students;
            });

            dataService.getData('/employees/getAssignees', {isEmployee: true, viewType: 'opportunities'}, function (employees) {
