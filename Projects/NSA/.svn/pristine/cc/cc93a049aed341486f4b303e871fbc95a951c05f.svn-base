define([
...
], function (Backbone, _, $, common, CONTENT_TYPES, VCONSTANTS, dataService, moment, ga, GA) {
    'use strict';
    ...
    var changeContentViewType = function (event, contentType, collection) {
        ...
        viewType = $(event.target).attr('data-view-type');
         url = '#erp/' + this.contentType + '/' + viewType + (browserFilter ? '/filter=' + browserFilter : '');

        if (id) {
            if (viewType !== 'list' && (viewType !== 'kanban') && (viewType !== 'datelist')) {
                url += '/' + id ;
            }
        ...
        if (id && (viewType === 'list') && (this.contentType === 'Tasks')) {
            ...
            if (kanbanFilter.project) {
                ...
                url = '#erp/' + this.contentType + '/' + viewType + '/' + kanbanFilter.project.value[0];
            } else {
                url = '#erp/' + this.contentType + '/' + viewType;
            }
        }
        ...
        if (App.currentViewType === null) {
            if (option) {
                switch (option.contentType) {
                    ...
                    case CONTENT_TYPES.BUNDLERETURNS:
                        //For NSA
                    case VCONSTANTS.VCOURSES:
                    case VCONSTANTS.VBATCHES:
                    case VCONSTANTS.VFACULTYMANAGEMENT:
                    case VCONSTANTS.VFRANCHISE:
                    case VCONSTANTS.VEXAMSCHEDULE:
                    case VCONSTANTS.VPRACTICEEXAM:
                    case VCONSTANTS.VUSERMANAGEMENT:
                    case VCONSTANTS.VATTENDANCE:
                    case VCONSTANTS.VLEAVE:
                    case VCONSTANTS.VNOTIFICATIONS:
                    case VCONSTANTS.VEVENTMANAGEMENT:
                    case VCONSTANTS.ENROLLMENT:
                    case VCONSTANTS.FEEDBACK:
                    case VCONSTANTS.VQUESTIONBANK:
                    case VCONSTANTS.VDASHBOARDREPORTS:
                    case VCONSTANTS.VCOURSEREPORTS:
                    case VCONSTANTS.VSLEAVEREPORTS:
                    case VCONSTANTS.VELEAVEREPORTS:
                    case VCONSTANTS.VLEADSREPORTS:
                    case VCONSTANTS.BUNDLERETURNS:
                    case VCONSTANTS.BUNDLEMOVE:
                    case VCONSTANTS.FACULTYLOGS:
                    case VCONSTANTS.SUBJECTSCHEDULE:
                    case VCONSTANTS.SCHOOLONBOARD:
                    case VCONSTANTS.SUBJECTTERMTOPICS:
                    case VCONSTANTS.TESTCONFIGURATION:
