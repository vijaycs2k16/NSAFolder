define([
    ...
    'libs/datatable/pdfmake/pdfmake.min',
    'libs/datatable/pdfmake/vfs_fonts.min',
    'libs/datatable/buttons.html5.min',
    'libs/datatable/buttons.print.min'
    ...
    var getStudentAttendanceDetails = function (filter, callback) {
        dataService.getData('/vsleave/std', {filter: filter, id: App.userName ? App.userName ? App.userName.id : null : null, cid: App.userName ? App.userName.cid : null, bid: App.userName ? App.userName.bid : null, center : App.userName ? App.userName.center : null},
            function (response) {
                callback(response);
            });
    };

    var getEmployeeAttendanceDetails = function (filter, callback) {
        dataService.getData('/vacation?year=2018', {filter: filter},
            function (response) {
                callback(response);
            });
    };

    var getStudentFeeDetails = function (filter, callback) {
        dataService.getData('/vregister/student', {filter: filter, id: App.userName ? App.userName ? App.userName.id : null : null},
            function (response) {
                callback(response);
            });
    };

    var getOrdersByFranchise = function (filter, fullfillment, register, callback) {
        dataService.getData('/orders/byCenter', {filter: filter, fullfillment: fullfillment, registeration: register},
            function (response) {
                callback(response);
            });
    };

    var getRegisterations = function (filter, fullfillment, callback) {
        dataService.getData('/vstudents/getRegisterationsByCenter', {filter: filter, fullfillment: fullfillment},
            function (response) {
                callback(response);
            });
    };

    var getLeadsByEmp = function (filter, opp, profile,  callback) {
        dataService.getData('/leads/overall/report', {filter: filter, opp: opp, profile: profile},
            function (response) {
                callback(response);
            });
    };
    var getLeadsByEmployee = function (filter,  callback) {
        dataService.getData('/leads/emp/report', {filter: filter},
            function (response) {
                callback(response);
            });
    };

    var getLeadsRevenueByCourse = function (filter,  callback) {
        dataService.getData('/leads/course/revenue', {filter: filter},
            function (response) {
                callback(response);
            });
    };

    var getHcReport = function (filter, opp,  callback) {
        dataService.getData('/leads/hc/report', {filter: filter, opp: opp},
            function (response) {
                callback(response);
            });
    };

    var getLeadsByProfile = function (filter, opp, profile, callback) {
        dataService.getData('/leads/profile/report', {filter: filter, opp: opp, profile: profile},
            function (response) {
                callback(response);
            });
    };

    var getStocksAvailable = function (filter, fullfillment, callback) {
        dataService.getData('/products/stocksAvalaible', {filter: filter},
            function (response) {
                callback(response);
            });
    };

    var getRevenueByFranchise = function (filter, isPendingFee, callback) {
        dataService.getData('/vstudents/revenueByCenter', {filter: filter, isPendingFee: isPendingFee},
            function (response) {
                callback(response);
            });
    };

    var getCoursesByFranchise = function (filter, isPendingFee, callback) {
        dataService.getData('/vbatch/getCB', {filter: filter, isPendingFee: isPendingFee},
            function (response) {
                callback(response);
            });
    };
    ...
    var getCurrentuser = function (callback) {
        dataService.getData('/users/current', {},
            function (response) {
                callback(response.user ? response.user: {});
            });
    };

    var studentTodayTopics = function (filter, callback) {
        dataService.getData('/vbatchSchedule/batch', {filter: filter, bid: App.userName ? App.userName.bid : null},
            function (response) {
                callback(response);
            });
    };

    var studentAssessmentDetails = function (filter, callback) {
        dataService.getData('/vAssessment/exam', {filter: filter},
            function (response) {
                callback(response);
            });
    };

    var employeeAssessmentDetails = function (filter, callback) {
        dataService.getData('/vAssessment/center', {filter: filter},
            function (response) {
                callback(response);
            });
    };

    var getStudentNotifications = function (filter, callback) {
        dataService.getData('/vnotifications/student', {filter: filter, id: App.userName ? App.userName.id : null},
            function (response) {
                callback(response);
            });
    };

    var getEmployeeNotifications = function (filter, callback) {
        dataService.getData('/vnotifications/emp', {filter: filter},
            function (response) {
                callback(response);
            });
    };

    var getEmployeeTopics = function (filter, callback) {
        dataService.getData('/vbatchSchedule/faculty', {filter: filter},
            function (response) {
                callback(response);
            });
    };

    var getTopStudents = function (filter, callback) {
        dataService.getData('/vassessment/exam/result/', {},
            function (response) {
                callback(response);
            });
    };
    ...
    var getQuestionBySubject = function (filter, forSales, callback) {
        dataService.getData('/vassessment/subject', {filter: filter},
            function (response) {
                callback(response);
            });
    };

    var getQuestionByTopic = function (filter, forSales, callback) {
        dataService.getData('/vassessment/topic', {filter: filter},
            function (response) {
                callback(response);
            });
    };

    var geStudentByExam = function (filter, mode, callback) {
        dataService.getData('/vassessment/report/exam', {filter: filter, mode: mode},
            function (response) {
                callback(response);
            });
    };
    ...
    var getExamByMode = function (filter, forSales, callback) {
        dataService.getData('/vassessment/exam/conduct', {filter: filter},
            function (response) {
                callback(response);
            });
    };
    ...
    var studentCourseDetails = function (filter, forSales, callback) {
        dataService.getData('/users/details', {
            filter     : filter,
            forSales   : forSales,
            id         : App.userName ? App.userName.id : null ,
            viewType   : 'list',
            count      : 5,
            sort       : {invoiceDate: -1},
            contentType: forSales ? 'invoice' : 'purchaseInvoices'
    ...
    var employeeCourseDetails = function (filter, forSales, callback) {
        dataService.getData('/users/details', {
            filter     : filter,
            forSales   : forSales,
            viewType   : 'list',
    ...
    var buildAllAphabeticArray = function (contentType) {
        ...
        var notNumbers = ['Persons', 'Employees', 'Academics'];
        var i;
    ...
    var getStudentImages = function (ids, url, callback) {
        dataService.getData(url, {ids: ids}, function (response) {
            if (response.data) {
                response.data.forEach(function (item) {
                    if (ids['task_id']) {
                        $("#" + ids['task_id'] + " img").attr("src", item.studentImageName);
                    }
                    $("#" + item._id + " img").attr("src", item.studentImageName);
                    $("#monthList #" + item._id + " img").attr("src", item.studentImageName);
                    $(".avatar.right[data-id='" + item._id + "'] img").attr("src", item.studentImageName);
                    $(".avatar.left[data-id='" + item._id + "'] img").attr("src", item.studentImageName);
                    $(".avatar-small.right[data-id='" + item._id + "'] img").attr("src", item.studentImageName);
                    if (item.studentImageName == "") {
                        $(".avatar-small.right[data-id='" + item._id + "'] img").hide();
                    }
                });
            }
    ...
    var datatableInitWithoutExport = function (id) {
        setTimeout(function () {
            $('#' + id).DataTable( {
                "ordering": false,
                "destroy": true,
    ...
    var datatableInitWithColFilter = function (id) {
        setTimeout(function () {

            $('#' + id + ' tfoot th').each( function () {
                var title = $(this).text();
                if(title) {
                    $(this).html( '<input type="text" class="datatable_input" placeholder="Search '+title+'" />' );
                }
    ...
    var getAllTasksByPerson = function (filter, callback) {
        dataService.getData('dealTasks/tasks/all', {filter: filter}, function (response) {
            callback(response);
        });
    };

    var getStagesByType = function (filter, typeId, callback) {
        dataService.getData('dealTasks/tasks/stages', {filter: filter, typeId: typeId}, function (response) {
            callback(response);
        });
    };

    var getTaskByType = function (filter, profileId, typeId, callback) {
        dataService.getData('dealTasks/tasks/profile', {filter: filter, typeId: typeId, profileId: profileId}, function (response) {
            callback(response);
        });
    };
    ...
    return {
        ...
        datatableInitWithoutExport        : datatableInitWithoutExport,
        datatableInitWithColFilter        : datatableInitWithColFilter,
        ...
        getImages                         : getImages,
        getStudentImages                  : getStudentImages,
        ...
        getQuestionsBySubject             : getQuestionBySubject,
        getQuestionByTopic                : getQuestionByTopic,
        getExamByMode                     : getExamByMode,
        geStudentByExam                     : geStudentByExam,
        ...
        getOrdersByFranchise              : getOrdersByFranchise,
        getRegisterations                 : getRegisterations,
        getLeadsByEmp                     : getLeadsByEmp,
        getLeadsByProfile                     : getLeadsByProfile,
        getStocksAvailable                : getStocksAvailable,
        getRevenueByFranchise             : getRevenueByFranchise,
        getCoursesByFranchise             : getCoursesByFranchise,
        studentTodayTopics                : studentTodayTopics,
        getStudentNotifications           : getStudentNotifications,
        getEmployeeNotifications          : getEmployeeNotifications,
        getEmployeeTopics                 : getEmployeeTopics,
        getTopStudents                    : getTopStudents,
        studentCourseDetails              : studentCourseDetails,
        employeeCourseDetails             : employeeCourseDetails,
        getStudentFeeDetails              : getStudentFeeDetails,
        getStudentAttendanceDetails       : getStudentAttendanceDetails,
        getEmployeeAttendanceDetails      : getEmployeeAttendanceDetails,
        studentAssessmentDetails          : studentAssessmentDetails,
        employeeAssessmentDetails         : employeeAssessmentDetails,
        getLeadsByEmployee                : getLeadsByEmployee,
        getLeadsRevenueByCourse           : getLeadsRevenueByCourse,
        getHcReport                       : getHcReport,
        getCurrentuser                    : getCurrentuser,
        getAllTasksByPerson               : getAllTasksByPerson,
        getStagesByType                   : getStagesByType,
