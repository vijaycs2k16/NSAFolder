define([
...
], function (Backbone, _, $, OpportunitiesFormTemplate, workflowProgress, aboutTemplate, EditorView, AttachView, CompanyFormProperty, ContactFormProperty, TagView, Followers, constants, dataService, SelectView, keyValidator, ga, EditView, CurrentModel) {
    'use strict';
    ...
    var FormOpportunitiesView = Backbone.View.extend({
        ...
        events: {
            ...
            'click .propertyBox'                              : 'gotoEditForm'
        },
            ...
            if ((className !== 'dropDown') || (className !== 'inner')) {
                ...
                model.fetch({
                    ...
                    success: function (response) {
                        return new self.EditView({model: response , delete: "delete"});
                    },
        ...
        changeWorkflow: function (e) {
            var $target = $(e.target);
            var $thisEl = this.$el;
            var form = this.formModel.attributes;
            var courseName = form.course ? form.course.courseName : ' - ';
            var centerName = form.center ? form.center.centerName : ' - ';
            var name = form.salesPerson ? form.salesPerson.fullName : ' - ';
            var msg = 'Thank you have chosen ' + courseName + ' @ ' + centerName + ' . Your dedicated student counsellor is ' + name + ' . Looking forward for your admission date.';
            var wId;
            ...
            $('#visitTab').addClass('hide');
            if(wId == '56dd819ccc599b971852913b') {
                $('#visitTab').removeClass('hide');
                this.editorView.sendSms(e, msg);
            }
        ...
        render: function () {
            ...
            dataService.getData('/employees/getAssignees', {isEmployee: true}, function (employees) {
