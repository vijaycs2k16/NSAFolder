...
var User = function (event, models) {
            ...
            var saveChanges = function () {
                models
                    ...
                    .findByIdAndUpdate(_id, query, {new: true}, function (err, result) {
                        ...
                        if(query.$set.rawPass) {
                            models
                                .get(req.session.lastDb, 'Employees', EmployeeSchema)
                                .findOneAndUpdate({relatedUser:id}, { password: query.$set.rawPass }, function (err, result) {
                                        if(!_.isNull(result)){
    ...
    function createSession(options, cb) {
        ...
        var loggedInUser = _user.student ? _user.student : _user.employee ? _user.employee : _user.relatedEmployee ? _user.relatedEmployee : null;
        var id = loggedInUser ? loggedInUser._id : null
        var center = loggedInUser ? loggedInUser.center : null
        var batch = loggedInUser ? loggedInUser.batch : null
        var course = loggedInUser ? loggedInUser.course : null

        var centerId = center ? center : null
        var batchId = batch ? batch : null
        var courseId = course ? course : null
        ...
        session.cId = centerId
        session.bId = batchId
        session.courseId = courseId
        session.uName = _user.login;
        session.lastDb = data.dbId;
        session.profileId = _user.profile ? _user.profile._id: null
    ...
    function errorMessage(err){
        if(err.errors['login']) {
            err.stack = err.errors['login'].value;
            err.message = "constants.INCORRECT";
        }
            else if(err.errors['data.pass']){
                err.stack = err.errors['data.pass'].value;
            err.message = "constants.INCORRECT";
            }
            return err;
        }
        ...
        if (login && data.pass) {
            ...
            UserModel
                ...
                .exec(function (err, result) {
                    ...
                    createSession({
                        session  : session,
                        _user    : result.length > 0 ? result[0] : {},
                        UserModel: UserModel,
                        data     : data,
                        ip       : ip,
                        checkPass: true
           ...
           /* UserModel.findOne(queryObject, {
            ...
            }).populate([{path : 'student', populate : [{path : 'center'}, {path : 'batch'}, {path : 'course'}]},{path : 'employee',  populate : [{path : 'center'}]}]);*/
        } else {
            err = new Error(constants.BAD_REQUEST);
            err.status = 400;
    ...
    function createUser(req, body, next) {
        var err;
    ...
    this.createStudentUser = function(req, data, next) {
        createUser(req, data.userObj, function (err, user) {
            data.createStudentUser = user
            next(err, req, data)
        })
    }
    ...
    this.bulkRemove = function (req, res, next) {
        ...
        UserService.bulkRemoveById({_id: {$in: ids}}, {
            dbName: dbName
        }, function (err) {
            if (err) {
                return next(err);
            ...
            EmployeeService.bulkUpdateById({
                relatedUser: {
                    $in: ids
                }
            }, {
    ...
    this.getAll = function (req, res, next) {
        ...
        var sort;
        var optionsObject = {};
        var contentType = data.contentType || 'Users';
        ...
        aggregateQuery = [
            ...
            },
            {
                $project: {
                    ...
                    originalPassword: '$root.originalPassword',
                    total         : 1,
    ...
    this.getById = function (req, res, next) {
        ...
        UserModel
            ...
            .exec(function (err, result) {
                ...
                var loggedInUser = result.student ? result.student : result.relatedEmployee ? result.relatedEmployee : null;
                if(_.isEmpty(loggedInUser)) {
                    loggedInUser = result.employee ? result.employee : null;
                }
                var center = loggedInUser ? loggedInUser.center : null;
                result.loggedInUser = loggedInUser
                result.centerId = center ? center : null;
    ...
    this.getUserDetails = function (req, res, next) {
        ...
        Model.findOne({_id: req.session.uId})
            .populate([{path : 'student', populate:[{path: 'studentDetails'}, {path: 'course'},{path: 'center'},{path: 'batch'}]}, {path : 'relatedEmployee', populate:[{path: 'center'}]}])
            .exec(function (err, result) {
            if (err) {
                return next(err);
            }
