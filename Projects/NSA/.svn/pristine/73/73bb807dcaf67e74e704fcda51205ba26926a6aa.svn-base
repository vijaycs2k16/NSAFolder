...
var FilterMapper = function () {
    var FILTER_CONSTANTS = require('../../common/constants/filters');
    var startDate;
    var endDate;
    ...
    this.mapDateFilter = function (filter, options) {
        var filterNames = Object.keys(filter);
        var contentType = options.contentType;
        var fieldsArray = options.keysArray;
        var withoutState = options.withoutState;
                        ...
                        if (typeof filterBackend === 'string') {
                            ...
                            if (!andState) {
                                ...
                                if(contentType == 'Transactions') {
                                    ...
                                    var val = []
                                    if (filterValues[0]) {
                                        startDate = moment(new Date(filterValues[0])).startOf('day');
                                        val.push(new Date(startDate));
                                    }

                                    if (filterValues[1]) {
                                        endDate = moment(new Date(filterValues[1])).endOf('day');
                                        val.push(new Date(endDate));
                                    }
                                    //filterResObject.push(filterValues)
                                }
