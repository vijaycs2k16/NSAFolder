define([
    ...
    var VacationListView = Backbone.View.extend({
        ...
        selectDepartment: function (id) {
            ...
            var self = this;
            this.$el.find('#department').text('Select department');
        ...
        vacationTypeForDD: function (content) {
            var array = ['Clear', 'Vacation', 'Medical', 'Half' , 'Sick', 'Education'];
            var firstChar;
        ...
        filterStudentsForDD: function (content) {
            ...
            if(centerId !== '' && departmentsId !== '' ) {
                dataService.getData('/Vacation/leave', {centerId: centerId, departmentsId: departmentsId, month:monthElement, year:yearElement}, function (employees) {
        ...
        renderdSubHeader: function ($currentEl) {
            ...
            $(subHeaderContainer[0]).attr('colspan', daysInMonth - 15);
        ...
        chooseOption: function (e) {
            ...
            function checkDay(element, selectedClass) {
                ...
                var vacDaysCount = tdTotalDays.text().trim() === '' ? 0 : +tdTotalDays.text();

                if (element.hasClass('V')) {
                    className = 'V';
                } else if (element.hasClass('P')) {
                    className = 'P';
                } else if (element.hasClass('M')) {
                    className = 'M';
                ...
                if (className) {
                    if (className !== selectedClass) {
                        ...
                        if (selectedClass === '' ) {
                            ...
                            (className == "H") ? vacDaysCount -= 0.5 : vacDaysCount -= 1;
                            employeesCount -= 1;
                        } else {
                            (className == "H") ? vacDaysCount += 0.5 : vacDaysCount;
                            (selectedClass == "H") ? vacDaysCount -= 0.5 : vacDaysCount;
        ...
        render: function () {
            ...
            dataService.getData('/franchise/', {category: 'CENTER'}, function (center) {
                ...
                if(App.currentUser.profile._id === 1522230115000) {
                    ...
                    self.selectDepartment(centerId);
                } else {
        ...
        getVacDaysCount: function (model, objs) {
            ...
            if (Array.isArray(objs) && !_.isEmpty(objs)) {
                var array = objs ? _.compact(objs[0].vacArray) : null;
                 var hArray = Lodash.filter(array,function (o) { return o == 'H' })
                model.vacArray = objs ? objs[0].vacArray : null;
                model.countVacationDays = array ? (array.length - hArray.length/2 ) : null;
            } else {
