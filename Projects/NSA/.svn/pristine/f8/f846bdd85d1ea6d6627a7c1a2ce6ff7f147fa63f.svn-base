   ...
   /* var student = new studentHandler(models);
    ...
    function saveQuestions(req, mergedObj, callback) {
        ...
        _.forEach(quesObjs, function (val, index) {
            console.log("sycbssssssssssss", val)
            var ques = {};
    ...
    function getSubjectData(req, callback) {
        console.log("5b2ba58c1460cc86e111425c")
    ...
    this.getExamBySub = function (req, res) {
        ...
        var limit = paginationObject.limit;
        var skip = paginationObject.skip;
        var findQuery = {$and: [{"practice.userName": headers.user_id}, {"practice.isSubmit": false}]};
        ...
        Model.aggregate([
            ...
            if (err) {
                events.emit('ErrorJsonResponse', req, res, {message: buildErrResponse(err, message.nsa22010)});
            } else {
                result = serviceUtils.parseDbObjectAsJSON(result);

                dateUtils.formatObjectsDates(result, ['dateBeginAhead', 'dateGenerated', 'dateCorrect', 'dateRead', 'dateSubmit'], 'type2', function (err, data) {
                    if (!_.isEmpty(result)) {
                        result[0].avgTime = !_.isEmpty(result) && !_.isEmpty(result[0].practice.questions) ? _.meanBy(result[0].practice.questions, function (o) {
                            return +o.attempted_time;
                        }) : 0
                    }
                    //res.status(200).send({sucess: true, data: getAvgTime(result)});
                    events.emit('JsonResponse', req, res, getAvgTime(result));
                })
            }
        });
    };
            ...
            _.map(dataObj, function (val, key) {
                ...
                _.forEach(val, function(value, index){
                    ...
                    if(index == val.length -1) {
                        ...
                        console.log("adta",typeof data.student);
