module.exports = (function () {
    var mongoose = require('mongoose');
    var ObjectId = mongoose.Schema.ObjectId;
    var StudentSchema = mongoose.Schema({
        studentName         : String,
        userName            : String,
        lastName            : String,
        studentEmail        : String,
        originalPassword    : String,
        studentPassword     : String,
        studentGender       : String,
        studentPhone        : Number,
        studentCity         : String,
        registrationNo      : String,
        oldRegNo            : String,
        age                 : Number,
        classDetail         : {type: ObjectId, ref: 'ClassDetails', default: null},
        section             : {type: ObjectId, ref: 'section', default: null},
        course              : {type: ObjectId, ref: 'Course', default: null},
        courseStartDate     : {type: Date, default:null},
        courseEndDate       : {type: Date, default:null},
        center              : {type: ObjectId, ref: 'Center', default: null},
        batch               : {type: ObjectId, ref: 'Batch', default: null},
        centerCourseFees    : Number,
        createdBy           : String,
        updatedBy           : String,
        followedBy          : {type: ObjectId, ref: 'Employees',default: null},
        createDate          : {type: Date, default:Date.now},
        createIp            : String,
        lastLoginTime       : {type: Date, default:null},
        studentImageName    : { type   : String,
            default: 'data:image/png;base64,'
        },
        cirularViewDateTime : {type: Date, default:null},
        studentStatus       : {type: Boolean, default: true},
        isDeleteStudent     : {type: Boolean, default: false},
        academicYear        : String,
        sourceOfEnquiry     : String,
        isRegistration      : {type: Boolean, default: false},
        studentDetails      : {type: ObjectId, ref: 'StudentDetails', default: null},
        stage               : {type: Object, default: null},
        defaultStudent      : {type: Boolean, default: false},
        interestCourse      : {type: ObjectId, ref: 'Course', default: null},
    }, {collection: 'Student'});

    mongoose.model('Student', StudentSchema);

    if (!mongoose.Schemas) {
        mongoose.Schemas = {};
    }

    mongoose.Schemas.Student = StudentSchema;
})();