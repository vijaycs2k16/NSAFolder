...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderSlatRowHtml: function() {
		...
		var isLabeled;
		var axisHtml;
		var subSlot = view.opt('viewSubSlotLabel');
		var slotDuration = 60;
		//var durationInMunites = moment.duration(slotDuration).valueOf();
		console.info('subSlot = ', subSlot);
		...
		while (slotTime < this.view.maxTime) {
			...
			/*Customized For NSA Ext Calendar */
					...
					(isLabeled ?
						'<span>' + // for matchCellWidths
							htmlEscape(slotDate.format(this.labelFormat)) +
							 ...
							 htmlEscape(slotDate.add(slotDuration, 'minutes').format(this.labelFormat)) : '') +
						'</span>' :
						''
						) +
				'</td>';
