define([
    ...
    'dataService',
    'Lodash'
], function (Backbone, $, _, NoteTemplate, onlyNoteTemplate, timelineTemplate, editNote, TaskModel, LeadNotificationModel, EditView, SelectView, CategoryView, moment, populate, CONSTANTS, dataService, Lodash) {
    'use strict';
    ...
    var NoteView = Backbone.View.extend({
        ...
        initialize: function (options) {
            ...
            this.leadNotModel = new LeadNotificationModel();
            this.taskModel.on('change:category', this.renderCategory, this);
        },

        events: {
            'focus .counterEl'                                 : 'checkCount',
            'click #noteArea, #taskArea, #smsArea, #emailArea, #visitArea'                       : 'expandNote',
        ...
        sendSms: function (e, smsTemp) {
            ...
            var saveObject = {}
            var users = [];
            var $description = $thisEl.find('#smsArea');
            var model = this.formModel ? this.formModel.toJSON() : {};
            var smsTemplateMsg = $.trim($description.val());
            var phoneNo = $('#phones_phone').val();
            phoneNo = phoneNo ? phoneNo : model.student ? model.student.studentPhone : '';
            ...
            var stage = $('#workflowProgress .tabListItem.active').find('span').attr('data-id')
            users.push({phoneNo: phoneNo})
            saveObject.users = users;
            saveObject.smsTemplateMsg = smsTemplateMsg;
            saveObject.lead = this.model.id;
            saveObject.status = true;
            var notify = {}
            notify.sms = true;
            saveObject.notify = notify
            saveObject.stage = stage;
            if(stage =='56dd819ccc599b971852913b' && smsTemp) {
                saveObject.smsTemplateMsg = smsTemp
            } else if(stage == '528ce779f3f67bc40b00001f' && smsTemp) {
                saveObject.smsTemplateMsg = smsTemp
            }
            this.leadNotModel.urlRoot = CONSTANTS.URLS.LEADSNOTIFICATIONS+ '?type=sms'

            this.leadNotModel.save(saveObject, {
                wait   : true,
                success: function () {
                    self.model.fetch({
                        success: function () {
                            self.leadNotModel = new LeadNotificationModel();
                            $thisEl.find('#smsArea').val('');
        ...
        sendEmail: function (e) {
            ...
            var saveObject = {}
            var users = [];
            var $description = $thisEl.find('#emailArea');
            var emailTemplateMsg = $.trim($description.val());
            var email = $('#email').val();
            email = email ? email : model.student ? model.student.studentEmail : '';
            ...
            var stage = $('#workflowProgress .tabListItem.active').find('span').attr('data-id')
            saveObject.emailTemplateMsg = emailTemplateMsg;
            saveObject.lead = this.model.id;
            saveObject.stage = stage;
            saveObject.email = email;

            this.leadNotModel.urlRoot = CONSTANTS.URLS.LEADSNOTIFICATIONS+ '?type=email'
        ...
        saveTask: function (dontCheck) {
            ...
            var taskType = $thisEl.find('#taskTypeDd').attr('data-id');
            ...
            var res = Lodash.filter(self.types, {'_id': $thisEl.find('#taskTypeDd').attr('data-id')});
            if(res.length > 0)
                var workflow = res[0].stages[0]._id;
            ...
            if (!taskType) {
                return App.render({
                    type: 'error',
                    message: 'Please select Tasktype'
                })
            }
        ...
        saveVisitTask: function (dontCheck) {
            var self = this;
            var $thisEl = this.$el;
            var assignedTo = $thisEl.find('#assigneToDd').attr('data-id');
            var $description = $thisEl.find('#visitArea');
            var description = $.trim($description.val());
            var dueDate = $.trim($thisEl.find('#visitDueDate').val());
            var time = moment($.trim($thisEl.find('#timepickerTwo').wickedpicker('time')).split(' '), 'hh:mm:ss A');
            if (dueDate) {
                dueDate = moment(dueDate).hours(time.get('hours')).minutes(time.get('minutes')).seconds(time.get('seconds')).toDate();
            }
            var saveObject = {
                assignedTo : assignedTo || '',
                taskType   : '5adecb1a35751f39a621038f',
                description: description,
                dueDate    : dueDate,
                workflow   : '5adebe33ad977705148e8785',
                category   : this.taskModel.get('category') ? this.taskModel.get('category')._id : null
            };
        ...
        render: function () {
            ...
            this.$el.find('#visitTab').addClass('hide');
            if(modelObj.workflow && modelObj.workflow._id =='56dd819ccc599b971852913b') {
                this.$el.find('#visitTab').removeClass('hide');
            }
            ...
            this.$el.find('#timepickerTwo').wickedpicker({
                showSeconds    : true, // Whether or not to show seconds,
                secondsInterval: 1, // Change interval for seconds, defaults to 1,
                minutesInterval: 1
            });

            populate.get2name('#assignedToDd', CONSTANTS.URLS.ASSIGNEES_PERSONSFORDD, {viewType: 'tasks'}, this);
            populate.get2name('#assigneToDd', CONSTANTS.URLS.ASSIGNEES_PERSONSFORDD, {viewType: 'tasks'}, this);
