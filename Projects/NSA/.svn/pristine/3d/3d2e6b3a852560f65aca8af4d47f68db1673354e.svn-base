define([
    ...
    'text!templates/VExamSchedule/EditTemplate.html',
    'common',
    'populate',
    'Validation',
    'vconstants',
    ...
    'dataService',
    'helpers'
], function (Backbone, _, $, Parent, EditTemplate, common, populate, Validation, CONSTANTS, SelectView, ga, examScheduleSerivce,moment, dataService, helpers) {
    var EditView = Parent.extend({
        ...
        chooseOption: function (e) {
            ...
            /*if (holder.attr('id') === 'center') {
                this.batchesResetForm();
                thisEl.find('#course').attr('data-id','');
        ...
        saveItem: function () {
            ...
            //data.section = thisEl.find('#sections').attr('data-id');
            var schedule = 'updateSchedule';
            data.schedule = schedule;
            data.classDetail  = this.classId;

            /*if(!data.section || data.section === 'Select'){
                 return App.render({
                     type   : 'error',
            ...
            if(offLine !== true){
                ...
                var courseObj = _.filter(this.config, {_id: paperId} );
                this.courseId = courseObj.length > 0 ? _.map(courseObj[0].course, '_id') : '';
                //data.course = this.courseId._id
        ...
        render: function () {
            ...
            var thisEl = this.$el;
            var data = {};
            this.classId  = this.currentModel.toJSON().paperConfig ? this.currentModel.toJSON().paperConfig.classDetail : '';
            ...
            dataService.getData('/vSection/limitSection/', {}, function (sections) {
                sections = _.map(sections, function (section) {
                    section.name = section.sectionName;
                    return section;
                });
                self.responseObj['#sections'] = sections;
            });

            this.selectBatch(self.selectCenters, this.courseId);

            this.$el.find('#examStartDate').datepicker({
                changeMonth: true,
                changeYear: true,
                minDate : new Date()
            });
