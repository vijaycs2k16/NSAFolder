...
var SchoolMediaLogSchema = mongoose.Schemas.SchoolMediaLogSchema;
...
exports.getAllUserNotificationLogs = function(req, res) {
    ...
    var Model = models.get(req.session.lastDb, 'SchoolMediaLog', SchoolMediaLogSchema);

    Model.find({'username': req.params.id, 'details.deactivated': false}).exec(function (err, result) {
        if (err) {
            return next(err);
        }
